...
// (async ()=>{
//   const res = await User.find({username:'violet'})
//   console.log(res)
// })();
...
exports.regUser = (req, res) => {
  ...
  //   }
  // )
  User.create({ username: userinfo.username, password: userinfo.password,email:userinfo.email },(err, result) => {
        if (err) return res.cc(err)
        //判断影响行数是否为一
        if (result.inserting) return res.cc('注册用户失败，请稍后再试！')
        //注册用户成功
        res.cc('注册成功！', 0)
      })
}
